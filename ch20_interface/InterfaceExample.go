...
func InterfaceExample() {
	...
	// 인터페이스에 구현할 메서드들만 넣고, 다른 구조체를 통해 메서드를 구현하면
	// 인터페이스에 해당 구조체를 대입해 사용할 수 있다!
	fmt.Printf("%s\n", stringer.String())
	fmt.Printf("%s\n", stringer.String2())
}
...
// 추상화란? -> 피호출 함수의 매개변수에 인터페이스를 정의하고, 호출부에서 해당 인터페이스를 구현하는
// 구조체의 객체를 전달할 수 있도록 하여 함수호출의 유연성을 높이는 기능.
